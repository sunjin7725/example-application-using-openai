...
class RedisClient:
    ...
    def embeddings_to_redis(self, embeddings: List[Embedding], index_name: str = "zelda_embeddings"):
        ...
        try:
            ...
            self.client.ft(index_name).create_index(
                fields=fields, definition=IndexDefinition(prefix=["embedding"], index_type=IndexType.HASH)
            )
        ...
        print(f"Loaded {self.client.info()['db0']['keys']} " f"documents in Redis Search index {index_name}")
    ...
    def search_redis(
        ...
        *,
        ...
        query = base_query.return_fields(*return_fields).sort_by("score").paging(0, k).dialect(2)

        params = {"vec_param": np.array(embedding_query, dtype=np.float32).tobytes()}
